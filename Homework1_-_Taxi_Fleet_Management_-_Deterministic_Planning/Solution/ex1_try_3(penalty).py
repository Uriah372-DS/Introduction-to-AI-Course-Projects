...
class TaxiProblem(search.Problem):
    ...
    def __init__(self, initial):
        ...
        self.fleet_capacity = sum([initial["taxis"][taxi_name]["capacity"] for taxi_name in initial["taxis"].keys()])
    ...
    def h(self, node):
        ...
        return max((self.n_taxis / self.fleet_capacity) * (self.h_1(node) + self.h_2(node)), self.h_turns_approx(node))
    ...
    def h_turns_approx(self, node):
        ...
        for passenger in undelivered_passengers:
            ...
            for taxi_name in self.state["taxis"].keys():
                ...
                min_border0 = min(self.state["passengers"][passenger]["location"][0],
                                  self.state["passengers"][passenger]["destination"][0])
                max_border0 = max(self.state["passengers"][passenger]["location"][0],
                                  self.state["passengers"][passenger]["destination"][0])
                min_border1 = min(self.state["passengers"][passenger]["location"][1],
                                  self.state["passengers"][passenger]["destination"][1])
                max_border1 = max(self.state["passengers"][passenger]["location"][1],
                                  self.state["passengers"][passenger]["destination"][1])

                penalty = sum([1 for i in range(min_border0, max_border0)
                               for j in range(min_border1, max_border1)
                               if self.state["map"][i][j] == "I"])

                min_dist = min(min_dist, taxi_to_passenger + passenger_to_destination + penalty)
